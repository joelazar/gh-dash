...
func shouldIncludeNotification(notification *Notification, filters map[string]string) bool {
	for key, value := range filters {
		switch key {
		case "saved":
			// Filter for saved/bookmarked notifications
			if value == "true" && !notification.Bookmarked {
				return false
			}
		case "done":
...
func BookmarkNotification(threadID string) error {
	log.Debug("BookmarkNotification", "threadID", threadID)
	
	client, err := NewClient()
	if err != nil {
		log.Debug("BookmarkNotification: failed to create client", "err", err)
		return err
	}
	
	endpoint := fmt.Sprintf("notifications/threads/%s/subscription", threadID)
	log.Debug("BookmarkNotification: calling PUT", "endpoint", endpoint)
	...
	// GitHub returns 200 OK with subscription JSON for PUT, but we don't need the response
	err = client.Put(endpoint, nil, nil)
	if err != nil {
		log.Debug("BookmarkNotification: PUT failed", "err", err)
		return err
	}
	
	log.Debug("BookmarkNotification: successfully bookmarked")
	return nil
}
			...
			if len(parts) == 2 {
				...
				case "is":
					...
					case "read":
						params["all"] = "true"
					case "saved":
						// GitHub API doesn't have native saved/bookmarked filter
